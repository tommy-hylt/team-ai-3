...
Copy a skill from this member to a target member, deploying it into all 3 skill folders (`.claude`, `.agents`, `.grok`).
...
Run the prepare script to ensure your `.claude`, `.agents`, and `.grok` copies of the skill are identical before copying:
...
This copies from your `.claude/skills/<skill-name>/` into the target member's `.claude/skills/<skill-name>/`, `.agents/skills/<skill-name>/`, and `.grok/skills/<skill-name>/`.
