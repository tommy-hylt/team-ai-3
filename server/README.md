...
`grok`'s `streaming-json` output also gets special-cased on the way *out*: it emits one JSON object per tiny token (`{"type":"text","data":"I"}`, `{"type":"text","data":"'ll"}`, ...), which if written to the per-request log file as-is produces thousands of one-word lines — unreadable, and the original symptom that prompted this. `executeAgent()` buffers grok's stdout and merges consecutive same-type chunks into one line every ~2s (and once more on process exit, to flush whatever's left) before appending to the log. This is purely cosmetic — the in-memory `stdout` used for the actual parsed response still accumulates every raw chunk immediately and is untouched by this. Other vendors don't stream at this granularity and are unaffected (log writes stay immediate for them).
