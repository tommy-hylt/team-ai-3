...
- **Grok (xAI)**: `grok-4.6` (currently the only Grok model this account's subscription provides as of 2026-09-09 — `grok-4.5` also still resolves per `grok models`, but wasn't kept as a second tier since prior grok version bumps in this repo have always been straight replacements, not side-by-side tiers; run `grok models` to check before assuming otherwise)
- **Codex (OpenAI)**: `codex-gpt-5.6-luna`, `codex-gpt-5.6-terra`, `codex-gpt-5.6-sol`, `codex-gpt-6-astra` (added 2026-09-09 — OpenAI's backend started listing `gpt-6-astra`, priority 1, "Our most capable model for complex, demanding work")

Superseded/removed names (kept here so a stray reference in old data is recognizable): `claude-haiku`/`claude-sonnet`/`claude-opus` (renamed to versioned names above), `claude-opus-4-8` (→ `claude-opus-5`, 2026-09-09 — confirmed via a direct `claude --model claude-opus-5 --print` probe that it's a recognized model on this account before migrating the 9 members that referenced it), `grok-build` (→ `grok-4.5`), `grok-4.5` (→ `grok-4.6`, 2026-09-09), `grok-composer-2.5-fast` (removed 2026-07-27 — `grok models` started reporting `"unknown model id"` for it, meaning the subscription no longer includes this tier; no members were referencing it at removal time), `codex-gpt-5.4`/`codex-gpt-5.4-mini`/`codex-gpt-5.3-codex` (→ the GPT-5.6 family above).

> **Codex CLI version matters, not just the model list.** On 2026-09-09, `~/.codex/models_cache.json` stopped including a `base_instructions` field the then-installed CLI (`0.144.1`) required, so every request logged repeated `failed to load models cache: missing field 'base_instructions'` / `failed to renew cache TTL: ...` errors — cosmetic (it just forced a network refetch each time instead of using the cache) but noisy, and coincided with OpenAI adding `gpt-6-astra` to the backend's model list. `npm view @openai/codex version` showed `0.153.4` already published; `npm install -g @openai/codex@latest` fixed it outright (confirmed via a clean `codex exec -m gpt-6-astra` probe with zero cache errors afterward). This is the second time an outdated local Codex CLI has silently fallen behind what this account's backend actually serves — if a Codex-based agent's log shows any `codex_models_manager` error, check `codex --version` against `npm view @openai/codex version` before assuming something else is wrong.
