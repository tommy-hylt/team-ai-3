...
Superseded/removed names (kept here so a stray reference in old data is recognizable): `claude-haiku`/`claude-sonnet`/`claude-opus` (renamed to versioned names above), `grok-build` (→ `grok-4.5`), `codex-gpt-5.4`/`codex-gpt-5.4-mini`/`codex-gpt-5.3-codex` (→ the GPT-5.6 family above).
